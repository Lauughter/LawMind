 ...
 * 这些工具通过调用 IntentClassifierEnhanced 和 LegalQueryExpander 来实现具体的功能逻辑，并且在执行过程中添加了错误处理和日志记录，确保在实际应用中能够稳定运行并提供有用的反馈信息。
 */
@Slf4j
@Component
public class LawIntentTools {
    ...
    public LawIntentTools(IntentClassifierEnhanced intentClassifierEnhanced,
    ...
    /**
     * 分析用户问题的法律意图类型，返回意图类别包括：LEGAL_CONSULTATION（法律咨询）、ARTICLE_LOOKUP（法条查询）、CASE_SEARCH（案例检索）、CALCULATION（金额计算）、DOCUMENT_DRAFTING（文书起草）、LEGAL_KNOWLEDGE（法律知识问答）。
    ...
    @Tool("分析用户问题的法律意图类型。" +
          ...
          "CASE_SEARCH（案例检索）、CALCULATION（金额计算）、DOCUMENT_DRAFTING（文书起草）、" +
          "LEGAL_KNOWLEDGE（法律知识问答）。" +
          "用于帮助后续检索更有针对性。")
    public String classifyLegalIntent(
            @P("用户的原始问题文本") String question) {
        try {
            IntentType intent = intentClassifierEnhanced.classifyType(question);
            int topK = intentClassifierEnhanced.adjustTopK(intent, 15);
            boolean deepRetrieval = intentClassifierEnhanced.useDeepRetrieval(intent);
            ...
            String suggestion = switch (intent) {
                ...
                case DOCUMENT_DRAFTING -> "建议走文书起草流程，生成模板并填充";
                case LEGAL_KNOWLEDGE -> "建议走知识问答检索，返回法律概念解释";
