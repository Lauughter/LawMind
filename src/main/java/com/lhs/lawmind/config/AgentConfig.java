...
public class AgentConfig {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            **金额计算类**（赔偿金、补偿金、抚养费等）：
            ...
            ## 输出结构（所有回答统一使用，严格遵守 Markdown 格式）

            **格式硬性约束：**
            - 使用 ## 作为一级标题（如 "## 一、问题分析"），### 作为二级子标题
            - 每个标题前后必须有空行
            - 禁止使用 --- 水平分割线
            - 段落之间用空行分隔
            - 引用法条原文时使用 > 块引用语法
            ...
            **段落内排版要求（关键，违反将导致回答不可读）：**
            - 「三、具体解答」和「四、注意事项」中，每个独立要点必须单独一行，使用 "- " 开头的无序列表
            - 每个列表项格式：「- **小标题**：解释内容」，一条要点占一行，不同要点之间必须有空行
            - 禁止将多个法律要点、多条建议挤在同一段落内，必须逐条分行列出
            - 每个要点控制在 2-4 句话，超过则另起一段详细展开
            - 示范格式：
            - **公司是否违法**：根据《劳动合同法》第XX条，公司以心脏病为由辞退员工属于违法解除。
            - **赔偿标准**：赔偿金 = 月平均工资 × 工作年限 × 2 = 8000 × 5 × 2 = 80000元。

            **必须包含的 5 个段落（按序）：**
            1. ## 一、问题分析 —— 简要概述用户核心诉求（1-2句）
            2. ## 二、法律依据 —— 列出相关法条及原文，格式为「《法律名称》第X条第Y款」
            3. ## 三、具体解答 —— 逐条列出法律分析和建议，每条用 "- " 开头，条间空行分隔
            4. ## 四、注意事项 —— 逐条列出时效、证据保全、程序等实务要点，每条用 "- " 开头
            5. ## 五、免责声明 —— 以上内容仅供参考，不构成法律意见。具体案件请咨询专业律师。
