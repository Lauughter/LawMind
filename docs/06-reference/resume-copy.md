...
> 类型：06-reference 历史资料 | 说明：个人简历文案，非项目文档，可按需删除
...
LawMind 是一个面向法律领域的智能问答平台，基于 Spring Boot + LangChain4j 构建，实现了从文档检索到多轮 Agent 推理的完整链路。系统支持法律知识检索、法条查询、金额计算、文书起草等场景，核心包含 RAG 多级混合检索管道、ReAct 多工具调用 Agent、上下文压缩、跨会话记忆系统等模块。
...
实现了完整的 LangChain4j ReAct Agent，支持 6 个法律工具自主调用，并通过门控系统实现「快慢分流」。
...
- **6 个法律工具**：searchLawKnowledge（带法律类型过滤）、getArticleText（精确法条原文）、classifyLegalIntent（意图分类 + 策略建议）、expandLegalQuery（术语扩展）、verifyCitation（引用事实核查）、retrieveMemory（跨会话记忆按需查询）。所有工具方法返回 String 并捕获全部异常，确保工具调用失败不中断 Agent 循环。
...
- **快慢分流**：简单法条查询走 Fast Channel（单次 LLM 调用，无工具，平均 <2 秒）；复杂咨询走 Agent Channel（6 工具 ReAct 循环）；文书起草走 Hybrid Channel。预计 60% 的请求走快速通道，大幅节省 API 成本和响应延迟。
...
> 面试用文档 | 基于项目实际代码编写 | 详细技术文档见 `docs/06-reference/project-tech-details.md`
